...
object LifeCodec {
 fun encode(l:LifeState)=JSONObject().apply{
  ...
  put("entries",JSONObject(l.entries))
 ...
 fun decode(j:JSONObject?):LifeState {
  ...
  return LifeState(j.optBoolean("welcomed"),plans,entries,j.optBoolean("quietEnabled"),LocalTime.parse(j.optString("quietStart","08:00")).toString(),LocalTime.parse(j.optString("quietEnd","16:00")).toString(),budget,reflections)
 }
}
