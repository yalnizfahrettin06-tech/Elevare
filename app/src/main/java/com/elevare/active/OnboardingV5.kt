...
@Composable fun TrainingOnboarding(store:Store){
 ...
 fun finish(days:Int){
  ...
  store.pauseTimer()
  store.update{it.copy(ready=true,onboardingVersion=5,age=age,focus=focus,recentGrowth=growth,sleepHabit=sleep,activityHabit=activity,dailyMinutes=minutes,safety=safety,trialDays=normalizeTrialDays(days))}
 }
 BackHandler(enabled=step>0){step=if(step==9)7 else step-1}
 if(step==8){PreparationScreen(answers,preparationElapsed,{preparationElapsed=it},onBack={step=7},onReady={step=9});return}
 if(step==9){TrialPreview(onFinish={finish(it)},onSkip={finish(0)},onBack={step=7});return}
 val valid=when(step){0->true;1->age in ProfileChoices.ages;2->focus in ProfileChoices.focus;3->growth in ProfileChoices.growth;4->sleep in ProfileChoices.sleep;5->activity in ProfileChoices.activity;6->minutes in ProfileChoices.minutes;else->answers.complete()}
 Column(Modifier.fillMaxSize()){
  ...
  key(step){Column(Modifier.weight(1f).verticalScroll(rememberScrollState()).padding(24.dp),verticalArrangement=Arrangement.spacedBy(16.dp)){
   when(step){
    ...
    1->{QuestionTitle("Kaç yaşındasın?");ProfileChoices.ages.toList().chunked(3).forEach{row->Row(horizontalArrangement=Arrangement.spacedBy(10.dp)){row.forEach{value->Box(Modifier.weight(1f)){Choice("$value",selected=age==value){age=value}}}}}
...
@Composable fun PreparationScreen(answers:TrainingAnswers,elapsed:Long,onElapsed:(Long)->Unit,onBack:()->Unit,onReady:()->Unit){
 ...
 Column(Modifier.fillMaxSize()){
  Row(Modifier.padding(16.dp),verticalAlignment=Alignment.CenterVertically){IconButton(onClick=onBack){Icon(Icons.Rounded.ArrowBack,"Yanıtlara dön")};Text("Rutin hazırlığı",fontWeight=FontWeight.Bold)}
  ...
  Column(Modifier.padding(24.dp)){BigButton(if(elapsed<PREPARATION_DURATION_MS)"Hazırlanıyor…" else "3 günlük denemeye geç",onReady,enabled=elapsed>=PREPARATION_DURATION_MS)}
 }
}
