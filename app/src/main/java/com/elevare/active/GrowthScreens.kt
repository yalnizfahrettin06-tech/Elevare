...
@Composable fun Choice(title:String,subtitle:String="",selected:Boolean,onClick:()->Unit){
 Surface(onClick=onClick,shape=RoundedCornerShape(14.dp),color=if(selected)MaterialTheme.colorScheme.secondaryContainer else MaterialTheme.colorScheme.surface,border=BorderStroke(1.dp,if(selected)MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.outlineVariant),modifier=Modifier.fillMaxWidth().semantics{this.selected=selected;role=Role.RadioButton}){
  Row(Modifier.heightIn(min=52.dp).padding(horizontal=16.dp,vertical=12.dp),verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(12.dp)){
   ...
   Icon(if(selected)ArcIcons.Checked else ArcIcons.Circle,null,tint=MaterialTheme.colorScheme.primary,modifier=Modifier.size(22.dp))
  }
 }
}
@Composable fun MenuRow(title:String,subtitle:String="",icon:ImageVector=ArcIcons.Chevron,onClick:()->Unit){
 Surface(onClick=onClick,color=MaterialTheme.colorScheme.surface,shape=RoundedCornerShape(16.dp),modifier=Modifier.fillMaxWidth()){
  Row(Modifier.heightIn(min=56.dp).padding(14.dp),verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(12.dp)){
   ...
   Icon(ArcIcons.Chevron,null,Modifier.size(20.dp))
  }
 }
}
 ...
 Surface(shape=RoundedCornerShape(16.dp),color=MaterialTheme.colorScheme.surface,modifier=Modifier.fillMaxWidth()){
  Column{
   Row(Modifier.fillMaxWidth().clickable{expanded=!expanded}.semantics{stateDescription=if(expanded)"Açık" else "Kapalı"}.heightIn(min=56.dp).padding(14.dp),verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(12.dp)){
    Icon(icon,null,tint=MaterialTheme.colorScheme.primary,modifier=Modifier.size(22.dp));Text(title,Modifier.weight(1f),fontSize=15.sp,fontWeight=FontWeight.Medium);Icon(if(expanded)ArcIcons.Up else ArcIcons.Down,null)
   }
...
@Composable fun TrialPreview(onFinish:(Int)->Unit,onSkip:()->Unit,onBack:(()->Unit)?=null){
 Column(Modifier.fillMaxSize()){
  Row(Modifier.padding(horizontal=20.dp,vertical=8.dp),verticalAlignment=Alignment.CenterVertically){if(onBack!=null)IconButton(onClick=onBack){Icon(ArcIcons.Back,"Geri")};Text("Elevare Pro",Modifier.weight(1f),fontSize=22.sp,fontWeight=FontWeight.Bold);Tag("DEMO")}
  Column(Modifier.weight(1f).verticalScroll(rememberScrollState()).padding(24.dp),verticalArrangement=Arrangement.spacedBy(22.dp)){
   Box(Modifier.size(64.dp).background(MaterialTheme.colorScheme.secondaryContainer,RoundedCornerShape(20.dp)),contentAlignment=Alignment.Center){Icon(ArcIcons.Spark,null,Modifier.size(30.dp),tint=MaterialTheme.colorScheme.primary)}
   Text("3 gün\nkendin için.",fontSize=36.sp,lineHeight=42.sp,fontWeight=FontWeight.Bold);QuietText("Rutinini keşfet, ilk antrenmanına başla.")
   Column(verticalArrangement=Arrangement.spacedBy(18.dp)){listOf("Kısa antrenmanlar","Çizimli hareket rehberi","Kaynaklı bilgi kartları").forEach{label->Row(horizontalArrangement=Arrangement.spacedBy(12.dp),verticalAlignment=Alignment.CenterVertically){Icon(ArcIcons.Check,null,tint=MaterialTheme.colorScheme.primary,modifier=Modifier.size(20.dp));Text(label,fontSize=15.sp)}}}
  }
  Column(Modifier.padding(horizontal=24.dp,vertical=14.dp),verticalArrangement=Arrangement.spacedBy(10.dp),horizontalAlignment=Alignment.CenterHorizontally){QuietText("Demo: ücret alınmaz, abonelik başlamaz.");BigButton("3 gün ücretsiz dene",{onFinish(TRIAL_DAYS)},icon=ArcIcons.Spark);TextButton(onClick=onSkip){Text("Şimdilik atla")}}
 }
}
...
@Composable fun GrowthHome(s:UserState,onWorkout:(String)->Unit,onStart:(String)->Unit,onSleep:()->Unit,onArticle:(String)->Unit,onResumePlan:()->Unit,onReview:()->Unit){
 ...
 PageColumn{
  ...
  MenuRow("Geceyi planla","${s.bed} · ${if(s.reminders)"Hatırlatıcı açık" else "Hatırlatıcı ayarla"}",ArcIcons.Moon,onSleep)
 }
}
...
@Composable fun EvidenceCard(e:Evidence,onClick:()->Unit){
 Surface(onClick=onClick,shape=RoundedCornerShape(16.dp),color=MaterialTheme.colorScheme.secondaryContainer,modifier=Modifier.fillMaxWidth()){
  Row(Modifier.padding(16.dp),verticalAlignment=Alignment.CenterVertically,horizontalArrangement=Arrangement.spacedBy(14.dp)){
   Icon(ArcIcons.Book,null,tint=MaterialTheme.colorScheme.primary,modifier=Modifier.size(24.dp));Column(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(5.dp)){Text(evidenceLabel(e.id),fontSize=16.sp,fontWeight=FontWeight.Medium);Text("Bilgi ve kaynak",fontSize=12.sp,color=MaterialTheme.colorScheme.onSurfaceVariant)};Icon(ArcIcons.Chevron,null,Modifier.size(20.dp))
  }
 }
}
...
@Composable fun EvidenceDetail(e:Evidence,onBack:()->Unit){
 ...
 PageColumn{
  ...
  ExpandSection("Uygulamadaki karşılığı",ArcIcons.Info){Text(e.takeaway);QuietText("Yayın özetine dayanır; kişisel tıbbi tavsiye veya ürün onayı değildir.")}
  OutlinedButton(onClick={try{c.startActivity(Intent(Intent.ACTION_VIEW,Uri.parse("https://pubmed.ncbi.nlm.nih.gov/${e.pmid}/")))}catch(_:Exception){}},modifier=Modifier.fillMaxWidth()){Text("PubMed kaynağını aç");Spacer(Modifier.width(8.dp));Icon(ArcIcons.External,null,Modifier.size(18.dp))}
 }
}
@Composable fun GrowthProfileHeader(store:Store,notify:(String)->Unit){
 ...
 SleepReminderLeadSettings(store,notify)
 QuietText("Yaklaşık ${normalizedSleepReminderLead(s.sleepReminderLeadMinutes)} dakika önce · ${sleepReminderTime(s)?:"—"}. Pil tasarrufu geciktirebilir.")
