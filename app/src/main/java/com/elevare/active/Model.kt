...
data class UserState(val ready:Boolean=false,val name:String="",val start:String=LocalDate.now().toString(),val favorites:Set<String> = emptySet(),val done:Map<String,Set<String>> = emptyMap(),val sessions:List<SessionLog> = emptyList(),val sleeps:List<SleepLog> = emptyList(),val bed:String="22:00",val wake:String="07:00",val reducedMotion:Boolean=false,val dark:Boolean=false,val haptic:Boolean=true,val gentle:Boolean=false,val pausedOn:String?=null,val pausedDays:Long=0,val active:ActiveSession?=null,val onboardingVersion:Int=0,val age:Int=0,val heightCm:Int=0,val targetCm:Int=0,val dailyMinutes:Int=5,val trialDays:Int=0,val reminders:Boolean=false,val focus:String="",val recentGrowth:String="",val sleepHabit:String="",val activityHabit:String="",val safety:String="",val voiceCoach:Boolean=true,val timerSound:String="countdown")
object Content{
 val moves=listOf(
 ...
 fun workout(id:String)=workouts.first{it.id==id}
}
